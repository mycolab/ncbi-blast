...
RUN gunzip -c edirect.tar.gz | tar xf - \
    && rm edirect.tar.gz \
    && cp -r edirect/* /usr/local/bin

# import sequences and create a BLAST database
RUN mkdir blastdb queries fasta results blastdb_custom

RUN efetch -db protein -format fasta \
...
RUN makeblastdb -in /blast/fasta/nurse-shark-proteins.fsa -dbtype prot \
